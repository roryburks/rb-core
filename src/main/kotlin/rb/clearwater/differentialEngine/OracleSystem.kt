...
class OracleSystemFactory(
    private val _animationSystem: IAnimationLoadingSystem,
...
class OracleSystem(
    ...
    private val animationAccess: IAnimationLoadingSystem,
    ...
    : IOracleSystem
{
    ...
    override fun tickUp(input: GameInputState,defaultToReplay: Boolean ): WorldState {
        ...
        val inputAccess = GameInputAccess(
            ...
            input)
        val newWorldState = processor.tick(canon[met-1].ws, inputAccess, animationAccess)
    ...
    override fun scryForward(input: GameInputState) {
        if( input == GameInputState(0) && canon.size > met){
            ...
            while (canon.size < met + 60)
            {
                ...
                val next = processor.tick(last.ws, GameInputAccess(previousInput, input), animationAccess)
        ...
        if(existing == null){
            ...
            for( i in 0 until 60){
                ws = processor.tick(ws, GameInputAccess(previousInput, input),animationAccess)
...
object OracleSystemFactoryProvider {
    val Factory = OracleSystemFactory(
        _animationSystem = AnimationLoaderSystemProvider.System,
